...
class ExcelDataObjectCreator:
    ...
    def __init__(self, file_path: str, process_limit: Optional[int] = None, eft_filter: Optional[List[str]] = None):
        """
        ...
        Args:
            ...
            eft_filter (List[str], optional): List of EFT numbers to filter for
        """
        self.file_path = Path(file_path)
        self.process_limit = process_limit
        self.eft_filter = set(eft_filter) if eft_filter else None
        self.df = None
    ...
    def _load_data(self):
        ...
        try:
            ...
            # Check for missing encounter/charge EFTs and remove them
            self._identify_and_remove_missing_encounter_charge_efts()
            
            # Apply EFT filter if provided
            if self.eft_filter:
                self._apply_eft_filter()

        except Exception as e:
            print(f"Error loading data: {e}")
            raise

    def _identify_and_remove_missing_encounter_charge_efts(self):
        """
        Identify EFTs with 'Encounter not found.' or 'Charge not found.' description and remove all rows for those EFTs.
        """
        print("🔍 Checking for EFTs with missing encounters/charges...")

        # Find all EFT NUMs that have "Encounter not found." or "Charge not found." in Description
        if 'Description' in self.df.columns:
            missing_descriptions = ["Encounter not found.", "Charge not found."]
            missing_mask = self.df['Description'].astype(str).str.strip().isin(missing_descriptions)
            missing_rows = self.df[missing_mask]

            if not missing_rows.empty:
                # Get unique EFT NUMs that have missing encounters/charges
                self.missing_encounter_efts = missing_rows['EFT NUM'].astype(str).unique().tolist()
                ...
                if self.missing_encounter_efts:
                    print(f"   ⚠️ Found {len(self.missing_encounter_efts)} EFTs with missing encounters/charges:")

                    # Show breakdown by description type
                    encounter_not_found_efts = missing_rows[missing_rows['Description'].astype(str).str.strip() == "Encounter not found."]['EFT NUM'].astype(str).unique()
                    charge_not_found_efts = missing_rows[missing_rows['Description'].astype(str).str.strip() == "Charge not found."]['EFT NUM'].astype(str).unique()

                    print(f"      • Encounter not found: {len(encounter_not_found_efts)} EFTs")
                    print(f"      • Charge not found: {len(charge_not_found_efts)} EFTs")

                    for eft in self.missing_encounter_efts:
                        # Determine which type of missing for this EFT
                        eft_descriptions = missing_rows[missing_rows['EFT NUM'].astype(str) == eft]['Description'].astype(str).str.strip().unique()
                        description_types = ", ".join(eft_descriptions)
                        print(f"        • EFT: {eft} ({description_types})")
                    ...
                    print(f"   🗑️ Removed {removed_row_count:,} rows from {len(self.missing_encounter_efts)} EFTs with missing encounters/charges")
                    print(f"   📊 Remaining rows: {len(self.df):,}")
                else:
                    print("   ✅ No EFTs with missing encounters/charges found")
            else:
                print("   ✅ No rows with missing encounter/charge descriptions found")
        else:
            print("   ⚠️ No 'Description' column found - skipping missing encounter/charge check")

    def _apply_eft_filter(self):
        """
        Apply EFT filter to keep only rows for specified EFT numbers.
        """
        print(f"🔍 Applying EFT filter for {len(self.eft_filter)} EFTs...")
        
        if 'EFT NUM' in self.df.columns:
            # Get current EFT numbers in the data
            current_efts = set(self.df['EFT NUM'].astype(str).unique())
            
            # Find which filter EFTs actually exist in the data
            existing_filter_efts = self.eft_filter.intersection(current_efts)
            missing_filter_efts = self.eft_filter - current_efts
            
            print(f"   ✅ Found {len(existing_filter_efts)} of {len(self.eft_filter)} filter EFTs in data: {', '.join(sorted(existing_filter_efts))}")
            if missing_filter_efts:
                print(f"   ⚠️ Missing {len(missing_filter_efts)} filter EFTs: {', '.join(sorted(missing_filter_efts))}")
            
            # Filter dataframe to only include rows for the specified EFTs
            original_row_count = len(self.df)
            self.df = self.df[self.df['EFT NUM'].astype(str).isin(self.eft_filter)].copy()
            filtered_row_count = len(self.df)
            
            print(f"   📊 Filtered from {original_row_count:,} rows to {filtered_row_count:,} rows")
        else:
            print("   ⚠️ No 'EFT NUM' column found - cannot apply EFT filter")
    ...
    def create_data_object(self) -> Dict:
        ...
        # Get all unique EFT NUMs (after missing encounter/charge EFTs have been removed)
    ...
    def _create_payment_object(self, payment_key: str, pmt_rows: pd.DataFrame) -> Dict:
        ...
        # Note: encs_to_check will be populated later by EncounterTagger, 
        # so we initialize the counts to 0 for now
        ...
        payment = {
            ...
            "total_encounters": total_encounters,  # Total number of encounters
            "encs_to_check_count": encs_to_check_count  # Will be updated by EncounterTagger
        }
    ...
    def get_missing_encounter_efts(self) -> List[str]:
        """
        Get the list of EFT NUMs that have missing encounters or charges.

        Returns:
            List[str]: List of EFT NUMs with missing encounters/charges
        """
    ...
    def create_stats_excel(self, output_path: str = None) -> str:
        """
        Create a stats Excel file with payment-level statistics.
        
        Args:
            output_path (str, optional): Path for the output file. If None, uses input file path with _stats suffix.
            
        Returns:
            str: Path to the created stats file
        """
        if not self.data_object:
            raise ValueError("No data object available. Run create_data_object() first.")
            
        # Determine output path if not provided
        if output_path is None:
            input_stem = self.file_path.stem.replace('_Scrubbed', '')
            output_path = self.file_path.parent / f"{input_stem}_stats.xlsx"
        
        print(f"📊 Creating stats Excel file: {output_path}")
        
        # Collect stats data
        stats_data = []
        
        for eft_num, eft in self.data_object.items():
        ...
        # Sort by EFT_NUM, then PMT_NUM
        stats_df = stats_df.sort_values(['EFT_NUM', 'PMT_NUM'])
        
        # Create Excel writer with openpyxl engine for formatting control
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            text_columns = ['PRACTICE', 'EFT_NUM', 'PMT_NUM', 'STATUS']
            
            # Find column indices for text formatting
            text_col_indices = []
            for col_name in text_columns:
                if col_name in stats_df.columns:
                    col_idx = stats_df.columns.get_loc(col_name) + 1  # Excel is 1-indexed
                    text_col_indices.append(col_idx)
            
            # Apply TEXT format to specified columns
            from openpyxl.styles import NamedStyle
            from openpyxl.utils import get_column_letter
            
            # Create a text format style
            text_style = NamedStyle(name="text_format")
            text_style.number_format = '@'  # '@' is the Excel TEXT format
            
            # Apply formatting to each text column
            for col_idx in text_col_indices:
                col_letter = get_column_letter(col_idx)
                
                # Format header
                worksheet[f'{col_letter}1'].style = text_style
                
                # Format all data cells in this column
                for row_idx in range(2, len(stats_df) + 2):  # +2 because Excel is 1-indexed and we have header
...
class AnalyticsProcessor:
        ...
        for eft_num, eft in data_object.items():
            ...
            if not eft.get("is_split", False):
                for payment_key, payment in eft["payments"].items():
                    ...
                    # Check if this payment has encounters to check (skip if none)
                    if encs_to_check_count > 0:
                        ...
                        # Check for No Status 22 Scenarios
                        self._analyze_no_status_22_scenarios(payment, payment_info, results)

                        # Only analyze Mixed Post payments for existing scenarios
    ...
    def _analyze_no_status_22_scenarios(self, payment: Dict, payment_info: Dict, results: Dict) -> None:
        """
        Analyze payments for "No Status 22 Scenarios" - payments that have encounters to check
        but no status 22 encounters, and are not split.
        ...
        # Check if any encounter has status 22
        has_status_22 = False
        for enc_key, enc_check_data in encs_to_check.items():
            if enc_check_data.get("clm_status", "").startswith("22"):
                has_status_22 = True
                break

        # If payment has encounters to check but no status 22, add it to results
        if not has_status_22 and len(encs_to_check) > 0:
            results["no_status_22_scenarios"].append(payment_info)
    ...
    def _analyze_charge_mismatch_encounters(self, payment: Dict, payment_info: Dict, results: Dict) -> None:
        ...
        Only adds the payment once with a count of charge mismatch encounters.
        ...
        """
        encs_to_check = payment.get("encs_to_check", {})
        charge_mismatch_encounters = []

        # Collect all encounters with charge mismatch
        for enc_key, enc_check_data in encs_to_check.items():
            ...
            if "chg_mismatch_cpt4" in encounter_types:
                charge_mismatch_encounters.append({
        ...
        # Only add the payment if it has charge mismatch encounters
        if charge_mismatch_encounters:
            encounter_info = {
                ...
                "charge_mismatch_count": len(charge_mismatch_encounters),
                "charge_mismatch_encounters": charge_mismatch_encounters
            }
    ...
    def _process_analytics_results(self, results: Dict) -> None:
        ...
        """
        # Sort No Status 22 scenarios by encounters to check count (descending)
        results["no_status_22_scenarios"].sort(key=lambda x: x["encs_to_check_count"], reverse=True)
        ...
        results["summary"] = {
            "no_status_22_scenarios_count": len(results["no_status_22_scenarios"]),
            ...
            "largest_no_status_22_encs": results["no_status_22_scenarios"][0]["encs_to_check_count"] if results["no_status_22_scenarios"] else 0,
    ...
    def print_analytics_summary(self) -> None:
        ...
        print(f"   • No Status 22 Scenarios: {summary.get('no_status_22_scenarios_count', 0)} payments")
        if summary.get('largest_no_status_22_encs', 0) > 0:
            print(f"     └─ Largest encounters to check: {summary.get('largest_no_status_22_encs', 0)}")
        ...
        print(f"   • Charge Mismatch CPT4 Encounters: {summary.get('charge_mismatch_cpt4_count', 0)} payments")
        ...
        if results["no_status_22_scenarios"]:
            top_no_status_22 = results["no_status_22_scenarios"][0]
            print(f"\n🔍 Largest No Status 22 Scenario:")
            print(f"   • EFT: {top_no_status_22['eft_num']}")
            print(f"   • Payment: {top_no_status_22['practice_id']}_{top_no_status_22['payment_num']}")
            print(f"   • Encounters to Check: {top_no_status_22['encs_to_check_count']}")
        ...
        if results["charge_mismatch_cpt4_encounters"]:
            ...
            print(f"   • Charge Mismatch Encounters: {smallest_charge['charge_mismatch_count']}")
            print(f"   • Total Encounters to Check in Payment: {smallest_charge['encs_to_check_count']}")
    ...
    def tag_encounters(self, data_object: Dict) -> Dict:
        ...
        for eft_num, eft in data_object.items():
            ...
            for payment_key, payment in eft["payments"].items():
                ...
                payment["encs_to_check_count"] = len(sorted_encs_to_check)
